...
//       }
    ...
    // 부품 검색
    static func fetchPartsMock() async throws -> [PartItem] {
        let url = "\(APIConfig.Inventory.baseURL)/getPartList"
        
        let response: PartResponse = try await NetworkManager.shared.request(url: url)
        return response.data.map(PartItem.init(dto:))
       }

    // 발주 생성
    static func createOrder(order: OrderCreateRequest) async throws {
        let url = "\(baseURL)"
